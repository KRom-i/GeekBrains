...
public class Product extends Service{
    ...
    public Product (int type, int numberGroup, String name, double cost, int balance, boolean timeControl, double addSumTimeControl) {
        this.Type = type;
    ...
    public Product (int id, int type, int numberGroup, String name, double cost, int balance, boolean timeControl, double addSumTimeControl) {
        this.Type = type;
        this.numberGroup = numberGroup;
        this.id = id;
        this.name = name;
            ...
            public void handle(MouseEvent mouseEvent) {
                if (mouseEvent.getClickCount() == 2) {
                    if (!service.isTimeControl()){
                        if (getBalance() == 999_999 || getBalance() > 0) {
                        ...
                        if (new DateTime().checkTime()) {
                            ...
                            new ErrorStage(String.format("Ограничение времени до %s.", DateControlService.getTimeControl()));
                        }
                    }
                }
            }
...
//                }
//            });
           int[] valueBalance = {1,5,10,25,50,75,100,150,200, 500, 1000, 5000, 100_000};
    ...
    public boolean addToDataBase(){
        ...
        try {
            ...
            String query = "INSERT INTO ServicePrice (" +
                           "ID_TYPE, ID_GROUP, NAME, COST, DELETE_CHECK, BALANCE, timeControl, addSumTimeControl) VALUES (?, ?, ?, ?, ?, ?, ?, ?);";
...
//                    ") VALUES (?, ?, ?, ?, ?, ?);";
            ...
            statement.setBoolean(7, this.isTimeControl ());
            statement.setDouble(8, this.getAddSumTimeControl ());
    ...
    public VBox getVBoxInfoPanel () {
        return vBoxInfoPanel;
    }

    public void setVBoxInfoPanel (VBox vBoxInfoPanel) {
        this.vBoxInfoPanel = vBoxInfoPanel;
    }
